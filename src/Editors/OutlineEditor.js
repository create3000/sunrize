...
module .exports = class OutlineEditor extends OutlineRouteGraph
{
   ...
   showContextMenu (event)
   {
      ...
      else if (element .is (".exported-node"))
      {
         ...
         var menu = [
            ...
            {
               label: _("Import Node..."),
               visible: executionContext !== this .executionContext && !element .closest (".instance-scene, .InlineGeometry.internal-scene") .length,
   ...
   async saveDataUrlToFile (id, executionContextId, nodeId)
   {
      ...
      const
         mimeType         = isData ? match [1] || "text/plain" : this .protocolToMimeType .get (protocol [0]),
      ...
      const response = await electron .ipcRenderer .invoke ("file-path", { type: "save", defaultPath, filters });
