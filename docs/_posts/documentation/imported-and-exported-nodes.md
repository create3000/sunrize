...
The imported node will then appear at the bottom of your scene. It is always possible to rename imported nodes, as well as to remove them. As the node has now been imported, it is now possible to create routes to and from this node, as well as references to it for use in the scene graph.
