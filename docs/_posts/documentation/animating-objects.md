...
- **Cutting:** Choose cut (or press *Ctrl-x* or *Delete*) to cut selected keyframes.
- **Copying:** Choose copy (or press *Ctrl-c*) to copy selected keyframes.
- **Paste** overlay at current time (*Ctrl-v*) pastes the selected keys at the current time. If the keys land at the same time as existing keys, the pasted keys replace the existing keys. If there is no new key to replace an existing key, the existing key is preserved (that is, you'll have a mixture of old and new keys).
